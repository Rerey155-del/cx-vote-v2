<x-admin-layout title="Update Candidate">
    ...
    @endsection
    <div class="px-12">
        <form action="{{ route('dashboard-candidate-update', $candidate->id) }}" method="post"
            ...
            <div class="my-6">
                ...
                <x-text-input id="wakil_name" class="block mt-1 w-full" type="text" name="wakil_name"
                    :value="old('wakil_name', $candidate->wakil_name)" required autocomplete="wakil_name" />
            ...
            <div class="my-6">
                <div class="form-group">
                    ...
                    <input id="misi" type="hidden" name="misi" value="{{ old('misi', $candidate->misi) }}">
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="w-16 h-16 flex items-center justify-center bg-gray-100 rounded overflow-hidden">
                                <img id="preview-image"
                                    src="{{ $candidate->image ? asset('storage/' . $candidate->image) : 'https://www.svgrepo.com/show/508699/picture.svg' }}"
                                    alt="Preview" class="w-16 h-16 object-cover" />
                            </div>
                            <div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const misi = document.querySelector('input#misi');
            const editor = document.querySelector('trix-editor[input="misi"]');

            if (misi && editor) {
                editor.editor.loadHTML(misi.value || '');
            }
        });
    </script>
</x-admin-layout>
